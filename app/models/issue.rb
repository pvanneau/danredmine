...
class Issue < ActiveRecord::Base
  ...
  # PIERRICK : les fonctionnalités et templates ne sont pas assignables
  def css_classes
    s = "issue status-#{status.position} priority-#{priority.position}"
    s << ' closed' if closed?
    s << ' not_assignable' if (tracker.id == 5 or tracker.id == 6)
  ...
  def save_issue_with_child_records(params, existing_time_entry=nil)
    Issue.transaction do
      ...
      # PIERRICK : status done = 100% et 100% = status done
      if params[:issue][:done_ratio] == "100"
        self.status_id = "3"
      end
      if params[:issue][:status_id] == "3"
        self.done_ratio = "100"
      end
